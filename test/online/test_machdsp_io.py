...
from vortex.io import MachDSPIO, MachDSPIOConfig, DAQmxIO, DAQmxConfig, daqmx
...
# configure MachDSP galvo output
mioc = MachDSPIOConfig()
mioc.divisor = 10
mioc.port = "COM6"
mioc.baud_rate = 1_000_000
mioc.samples_per_block = ac.records_per_block
mioc.blocks_to_buffer = PRELOAD_COUNT

for (i, c) in enumerate(mioc.output_channels):
    c.logical_units_per_physical_unit = 15 / 10
    c.stream = Block.StreamIndex.GalvoTarget
    c.channel = i

mio = MachDSPIO(gcl('machdsp', LOG_LEVEL))
mio.initialize(mioc)
