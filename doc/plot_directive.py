...
def setup(app):
    ...
    app.add_config_value('plot_variants', [({}, None)], True)
...
def render_figures(code, code_path, output_dir, output_base, context,
    ...
    *output_base*
    ...
    total_runs = len(code_pieces) * len(config.plot_variants)
    results = []
    for i, ((variant_rcparams, variant_class), code_piece) in enumerate(itertools.product(config.plot_variants, code_pieces)):
        images = []
        ...
        if not all_exists:
            break
        results.append((code_piece, images, variant_class))
    else:
        all_exists = True
    ...
    for i, ((variant_rcparams, variant_class), code_piece) in enumerate(itertools.product(config.plot_variants, code_pieces)):
        ...
        if not context or config.plot_apply_rcparams:
            clear_state({**config.plot_rcparams, **variant_rcparams}, close_figs)
        elif close_figs:
            plt.close('all')
        ...
        for j, figman in enumerate(fig_managers):
            if len(fig_managers) == 1 and total_runs == 1:
                img = ImageFile(output_base, output_dir)
            elif total_runs == 1:
                img = ImageFile("%s_%02d" % (output_base, j), output_dir)
            else:
...
def run(arguments, content, options, state_machine, state, lineno):
    ...
    total_lines = []
    for j, (code_piece, images, variant_class) in enumerate(results):
        ...
        if variant_class:
            opts.append(f':figclass: {variant_class}')
        ...
        result = jinja2.Template(config.plot_template or template).render(
            ...
            src_name=src_name,
            multi_image=len(images) > 1 or len(config.plot_variants) > 1,
            options=opts,
            srcset=srcset,
            images=images,
